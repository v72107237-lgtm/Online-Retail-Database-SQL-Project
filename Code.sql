...
*/
--CREATE DB 
CREATE DATABASE OnlinrRetailDB;
GO
...
EXEC sp_rename 'dbo.Orders.CustomerId','CustomerID','COLUMN';
...
('Sameer',' Khanna','sameer.khanna@example.com','123-456-7890','123 Elm St.','Springfirld','IL','62701','USA'),
('Jane',' Smith','jane.smith@example.com','234-567-8901','456 Oak St.','Madison','WI','53703','USA'),
('Harshad',' Patel','harshad.patel@example.com','123-456-7890','123 Elm St.','Springfirld','IL','62701','USA');
...
--Query1.retrive all orders for specific customer

SELECT O.OrderID,O.CustomerID,O.OrderDate,O.TotalAmount,OI.OrderItemID,P.ProductID,OI.Quantity,OI.Price,P.ProductName
FROM Orders AS O
...
SELECT AVG(TotalAmount) AS "AVG Orders" FROM Orders; 

--Query4.list the top 5 customers by total spending
...
--5.Retrive the most popular product category
SELECT CategoryID, CategoryName,rn,Popular
FROM( 
SELECT C.CategoryID, C.CategoryName, SUM(OI.Quantity) AS Popular,
...
SELECT C.CustomerID,C.FirstName,C.LastName,O.OrderDate,C.Email,C.Phone
FROM customers AS C
INNER JOIN Orders AS O ON C.CustomerID=O.CustomerID
WHERE O.OrderDate >= DATEADD(DAY,-30,GETDATE());

--Query8. Calculate the total number of ordres placed each month

SELECT YEAR(OrderDate) As OrderYear, MONTH(OrderDate) AS OrderMonth,COUNT(OrderDate) As TotalOrders
FROM ORDERS GROUP BY YEAR(OrderDate),MONTH(OrderDate)
ORDER BY OrderYear;

--Query9.Retrive the details of the most recent order

SELECT TOP 3 O.OrderID,O.OrderDate,O.TotalAmount,C.CustomerID,C.CustomerID,C.FirstName,C.LastName
FROM Orders AS O
...
INSERT INTO Customers(FirstName,LastName,Email,Phone,Address,City,State,ZipCode,Country) VALUES
('Amir',' Khann','amir.khan@example.com','245-676-867','123 Shi St.','Springfirld','IL','62701','USA');
...
--Query12.retrive total Quantity sold for each product .
 
 SELECT P.ProductID, P.ProductID, SUM(Quantity) AS TotalQuantity
 FROM Products AS P
...
SELECT P.ProductID, P.ProductName, COUNT(OI.OrderID) AS NoOfOrders 
FROM Products AS P 
INNER JOIN OrderItems AS OI ON P.ProductID = OI.ProductID
GROUP BY P.ProductID,P.ProductName
...
INSERT INTO OrderItems VALUES (OrderID, ProductID, Quantity, Price)
...
SELECT Country , COUNT(CustomerID) AS TotNoCustomer FROM Customers GROUP BY Country;
...
SELECT O.OrderID,C.CustomerID,C.FirstName,C.LastName,COUNT(OI.OrderItemID) AS NumOfItems
FROM OrderItems AS OI 
...
--DROP Foreign Key Constraint from OrderItems table BECAUSE TO GIVE MANUAL Clusterd IDX ON ProductID,WE have TO REMOVE Constraints ON IT
--Remove FK,Other Inbuilt Clustered idx ,Create new Clustered idx With name


ALTER TABLE OrderItems DROP Constraint FK__OrderItem__Produ__628FA481;
...
--Inbuilt primary idx should remove -> RC DELETE -> if not possible means it have references  so remove references
--,Create New Clustered idx ,Again add previously removed references to respective table

--here inbuilt idx have references hence removing it from orderItems(fk)

ALTER TABLE OrderItems DROP CONSTRAINT FK__OrderItem__Order__6383C8BA;
...
ALTER TABLE Orders DROP CONSTRAINT FK__Orders__Customer__5FB337D6;
...
SELECT TOP 5 TotalAmount , CustomerID, FirstName, LastName, TotalOrders FROM vw_CustomerOrders
ORDER BY TotalAmount DESC;
...
SELECT * FROM vw_ProductDeails WHERE Stock <= 1;
